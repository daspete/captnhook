            ...
            <form v-on:submit.prevent="OnSubmit">
                ...
                <div class="row">
                    <div class="col-12 text-right">
                        <button class="button" type="submit"><icon name="save"/><span>Update project</span></button>
                    </div>
                </div>
            </form>
        </div>
...
export default {
    async asyncData({ app, params }){
        let project = await app.$axios.$get(`projects?name=${ params.project }`);
        project = project[0];
        let branches = await app.$axios.$get(`branches?projectId=${ project.id }`);
    ...
    methods: {
        ...
        async OnSubmit(e){
            let project = await this.$axios.$put(`projects/${ this.project.id }`, this.project);
            let branches = await this.$axios.$get(`branches?projectId=${ project.id }`);
